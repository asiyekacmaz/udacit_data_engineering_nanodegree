...
def process_song_data(spark, input_data, output_data):
    ...
    artists_schema_table = spark.sql(""" select distinct a.artist_id, a.artist_name, 
...
def process_log_data(spark, input_data, output_data):
    ...
    time_schema_table = spark.sql(""" select t.ts as start_time, 
    hour(t.ts) as hour, 
    dayofmonth(t.ts) as day, 
    weekofyear(t.ts) as week, 
    month(t.ts) as month, 
    year(t.ts) as year, 
    dayofweek(t.ts) as weekday 
    from (select totimestamp(timestamp.ts/1000) 
    as ts from logs_table timestamp 
    where timestamp.ts IS NOT NULL ) t """)
...
def main():
    ...
    output_data = "s3://udacity-dend/log_data"
